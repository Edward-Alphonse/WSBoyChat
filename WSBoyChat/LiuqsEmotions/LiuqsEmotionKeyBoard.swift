...
class LiuqsEmotionKeyBoard: UIImageView ,UIScrollViewDelegate {
    ...
    func creatPageViewOneBtns() {
        ...
        for i:Int in 0..<emojicount {
            ...
            if (i == emojicount - 1) {
                ...
                btn.setImage(UIImage.init(named: "backDelete"), for: UIControl.State.normal)
                ...
                btn.addTarget(self, action: #selector(deleteBtnClick(btn:)), for: UIControl.Event.touchUpInside)
                ...
                btn.addTarget(self, action: #selector(insertEmoji(btn:)), for: UIControl.Event.touchUpInside)
            }
    ...
    func ceartPageViewTwoBtns() {
        ...
        for i:Int in 0..<emojicount {
            ...
            if (i == emojicount - 1) {
                ...
                btn.addTarget(self, action: #selector(self.deleteBtnClick(btn:)), for: UIControl.Event.touchUpInside)
                ...
                btn.addTarget(self, action: #selector(self.insertEmoji(btn:)), for: UIControl.Event.touchUpInside)
            }
    ...
    func creatGifBtns() {
        ...
        for i:Int in 0..<gifCount {
            ...
            imageView.setImage(UIImage.init(named: gifName), for: UIControl.State.normal)
            
            imageView.tag = i + 1
            
            imageView.addTarget(self, action: #selector(emotionGifClick(btn:)), for: UIControl.Event.touchUpInside)
    ...
    func createSendBtn() {
        ...
        sendBtn.setBackgroundImage(UIImage.createImageWithColor(color: BLUE_Color), for: UIControl.State.selected)
        
        sendBtn.setBackgroundImage(UIImage.createImageWithColor(color: UIColor.lightGray), for: UIControl.State.normal)
        
        sendBtn.isSelected = true
        
        sendBtn.tag = 44
        
        sendBtn.addTarget(self, action:  #selector(emotionBtnDidClick(btn:)), for: UIControl.Event.touchUpInside)
    ...
    func createEmotionBtns() {
        ...
        for i:Int in 0..<emotionBtnsCount {
            ...
            emotionBtn.addTarget(self, action: #selector(emotionBtnsClick(btn:)), for: UIControl.Event.touchUpInside)
            
            btnsBar.addSubview(emotionBtn)
            
            emotionBtn.setImage(UIImage.init(named: "compose_emoticonbutton_background"), for: UIControl.State.normal)
            
            emotionBtn.setImage(UIImage.init(named: "compose_emoticonbutton_background_highlighted"), for: UIControl.State.selected)
            
            emotionBtn.setBackgroundImage(UIImage.createImageWithColor(color: BACKGROUND_Color), for: UIControl.State.selected)
    ...
    //静态表情的按钮事件
    @objc func insertEmoji(btn:UIButton) {
    ...
    func resetTextStyle() {
        ...
        KeyTextView?.textStorage.removeAttribute(NSAttributedString.Key.font, range: wholeRange)
        
        KeyTextView?.textStorage.addAttributes([NSAttributedString.Key.font:font], range: wholeRange)
    ...
    //gif表情按钮点击事件
    @objc func emotionGifClick(btn:UIButton) {
    
        self.delegate?.gifBtnClick(btn: btn)
    }
    
    //发送，删除，表情等按钮的事件，用来处理按钮状态
    @objc func emotionBtnDidClick(btn:UIButton) {
    ...
    @objc func emotionBtnsClick(btn:UIButton) {
