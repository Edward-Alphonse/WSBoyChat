...
class LiuqsToolBarView: UIImageView {
    ...
    lazy  var toolBarEmotionBtn:UIButton = {
        ...
        emotionBtn.setImage(UIImage.init(named: "emotionBtn_no"), for: UIControl.State.normal)
        
        emotionBtn.setImage(UIImage.init(named: "emotionBtn_se"), for: UIControl.State.selected)
        
        emotionBtn.addTarget(self, action: #selector(emotionBtnDidClicked(btn:)), for: UIControl.Event.touchUpInside)
    ...
    @objc func emotionBtnDidClicked(btn:UIButton) {
